...
<?php
  try {
      ...
      $stmt = $conn->prepare("SELECT * FROM tbl_orders_a175838, tbl_staffs_a175838_pt2,
        tbl_customers_a175838_pt2, tbl_orders_details_a175838 WHERE
        tbl_orders_a175838.fld_staff_num = tbl_staffs_a175838_pt2.fld_staff_num AND
        tbl_orders_a175838.fld_customer_num = tbl_customers_a175838_pt2.fld_customer_num AND
        tbl_orders_a175838.fld_order_num = tbl_orders_details_a175838.fld_order_num AND
        tbl_orders_a175838.fld_order_num = :oid");
  ...
  <div class="col-xs-5">
    <div class="panel panel-default">
      <div class="panel-heading">
        <h4>From: Photocopiers & Copy Supplies Sdn. Bhd.</h4>
      </div>
...
<table class="table table-bordered">
 ...
 try {
        $conn = new PDO("mysql:host=$servername;dbname=$dbname", $username, $password);
        $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
          $stmt = $conn->prepare("SELECT * FROM tbl_orders_details_a175838,
            tbl_products_a175838_pt2 where 
            tbl_orders_details_a175838.fld_product_num = tbl_products_a175838_pt2.fld_product_num AND
            fld_order_num = :oid");
        $stmt->bindParam(':oid', $oid, PDO::PARAM_STR);
          $oid = $_GET['oid'];
        $stmt->execute();
